...
class TtCharacterContextAlignmentCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterContextAlignmentSetCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean =
        when (args.firstOrNull()?.lowercase()) {
            in setAliases -> setCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character context alignment [${subcommands.joinToString("|")}]")
                true
            }
        }
