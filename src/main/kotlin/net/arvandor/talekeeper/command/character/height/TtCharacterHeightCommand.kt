...
class TtCharacterHeightCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterHeightSetCommand(plugin)
    private val hideCommand = TtCharacterHeightHideCommand(plugin)
    private val unhideCommand = TtCharacterHeightUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character height [${subcommands.joinToString("|")}]")
                true
            }
        }
