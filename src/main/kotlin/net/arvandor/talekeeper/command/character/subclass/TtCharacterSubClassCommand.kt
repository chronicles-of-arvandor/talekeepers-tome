...
class TtCharacterSubClassCommand(private val plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterSubClassSetCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean =
        when (args.firstOrNull()?.lowercase()) {
            in setAliases -> setCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${ChatColor.RED}Usage: /character subclass [${subcommands.joinToString("|")}]")
                true
            }
        }
