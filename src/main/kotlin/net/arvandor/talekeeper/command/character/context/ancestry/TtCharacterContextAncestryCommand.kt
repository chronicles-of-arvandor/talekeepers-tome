...
class TtCharacterContextAncestryCommand(private val plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterContextAncestrySetCommand(plugin)
    private val hideCommand = TtCharacterContextAncestryHideCommand(plugin)
    private val unhideCommand = TtCharacterContextAncestryUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character context ancestry [${subcommands.joinToString("|")}]")
                true
            }
        }
