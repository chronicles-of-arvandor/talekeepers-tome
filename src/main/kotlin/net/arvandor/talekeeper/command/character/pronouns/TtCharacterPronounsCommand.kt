...
class TtCharacterPronounsCommand(private val plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val addCommand = TtCharacterPronounsAddCommand(plugin)
    private val removeCommand = TtCharacterPronounsRemoveCommand(plugin)
    private val setChanceCommand = TtCharacterPronounsSetChanceCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in setChanceAliases -> setChanceCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character pronouns [${subcommands.joinToString("|")}]")
                true
            }
        }
