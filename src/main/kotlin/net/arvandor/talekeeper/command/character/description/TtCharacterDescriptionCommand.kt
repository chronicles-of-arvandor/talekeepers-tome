...
class TtCharacterDescriptionCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterDescriptionSetCommand(plugin)
    private val extendCommand = TtCharacterDescriptionExtendCommand(plugin)
    private val hideCommand = TtCharacterDescriptionHideCommand(plugin)
    private val unhideCommand = TtCharacterDescriptionUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character description [${subcommands.joinToString("|")}]")
                true
            }
        }
