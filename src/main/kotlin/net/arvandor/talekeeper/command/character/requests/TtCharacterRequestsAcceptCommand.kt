...
class TtCharacterRequestsAcceptCommand(private val plugin: TalekeepersTome) : CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        asyncTask(plugin) {
            ...
            if (profile == null) {
                sender.sendMessage("${RED}No profile was found for that player.")
                return@asyncTask
            }
            ...
            notificationService.createNotification(
                profile,
                "${GREEN}Character creation request approved",
                "${GRAY}Your character creation request was approved by ${sender.name}. Use $GREEN/character context ${GRAY}to open the character creator.",
            )

            sender.sendMessage("${GREEN}Character creation request approved.")
        }
        return true
    }
