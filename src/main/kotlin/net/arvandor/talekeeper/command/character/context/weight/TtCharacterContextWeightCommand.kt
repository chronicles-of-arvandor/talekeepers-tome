...
class TtCharacterContextWeightCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterContextWeightSetCommand(plugin)
    private val hideCommand = TtCharacterContextWeightHideCommand(plugin)
    private val unhideCommand = TtCharacterContextWeightUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character context weight [${subcommands.joinToString("|")}]")
                true
            }
        }
