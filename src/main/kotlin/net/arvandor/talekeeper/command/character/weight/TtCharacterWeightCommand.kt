...
class TtCharacterWeightCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterWeightSetCommand(plugin)
    private val hideCommand = TtCharacterWeightHideCommand(plugin)
    private val unhideCommand = TtCharacterWeightUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character weight [${subcommands.joinToString("|")}]")
                true
            }
        }
