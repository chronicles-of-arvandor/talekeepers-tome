package net.arvandor.talekeeper.command.character.profile
...
class TtCharacterProfileCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val hideCommand = TtCharacterProfileHideCommand(plugin)
    private val unhideCommand = TtCharacterProfileUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character profile [${subcommands.joinToString("|")}]")
                true
            }
        }
