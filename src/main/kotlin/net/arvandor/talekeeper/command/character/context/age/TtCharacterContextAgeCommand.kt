...
class TtCharacterContextAgeCommand(private val plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterContextAgeSetCommand(plugin)
    private val hideCommand = TtCharacterContextAgeHideCommand(plugin)
    private val unhideCommand = TtCharacterContextAgeUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character context age [${subcommands.joinToString("|")}]")
                true
            }
        }
