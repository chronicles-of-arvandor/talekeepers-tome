...
class TtCharacterNameCommand(plugin: TalekeepersTome) : CommandExecutor, TabCompleter {

    private val setCommand = TtCharacterNameSetCommand(plugin)
    private val hideCommand = TtCharacterNameHideCommand(plugin)
    private val unhideCommand = TtCharacterNameUnhideCommand(plugin)
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>) =
        when (args.firstOrNull()?.lowercase()) {
            ...
            in unhideAliases -> unhideCommand.onCommand(sender, command, label, args.drop(1).toTypedArray())
                ...
                sender.sendMessage("${RED}Usage: /character name [${subcommands.joinToString("|")}]")
                true
            }
        }
