...
class TtHpDecreaseCommand(private val plugin: TalekeepersTome) : CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        asyncTask(plugin) {
            ...
            val cappedAmount = amount.coerceAtMost(character.hp + character.tempHp)
            ...
            sender.sendMessage("$RED-$cappedAmount HP")
    ...
    override fun onTabComplete(
        ...
        args.size == 2 -> (1..600).map(Int::toString).filter { it.startsWith(args[1], ignoreCase = true) }
        else -> emptyList()
    }
}
